...
public class ConversionApiController {
    ...
    public ConversionApiController(CurrencyConversionService conversionService) {
    ...
    public ResponseEntity<ConversionResponse> convert(@Valid @RequestBody ConversionRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(conversionService.convert(request));
    }

    @GetMapping("/historico")
    @Operation(summary = "Lista o historico", description = "Retorna todas as conversoes salvas, da mais recente para a mais antiga.")
    public List<ConversionResponse> getHistory() {
        return conversionService.getHistory();
    }

    @DeleteMapping("/historico")
    @Operation(summary = "Limpa o historico", description = "Remove todas as conversoes salvas no banco H2.")
